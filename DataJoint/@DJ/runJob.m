...
assert(isnumeric(jobNum) && mod(jobNum,1)==0 && jobNum>=0 && jobNum<=255, 'jobNum must be an integer between 0 and 255');
...
try
    ...
    ddj = addField( ddj, 'job_status', 'enum("reserved","completed","error","ignore")', 'reserved/completed/error/ignore. If tuple is missing, the job is available');
    ddj = addField( ddj, 'error_message', 'varchar(1023)', 'error message returned by the failed job', []);
    ddj = addField( ddj, 'error_stack', 'blob', 'error stack containing filenames and line numbers', []);
    jobDJ = execute( ddj );
...
fun = @(key) eval(sprintf('job%d(dj,key)',jobNum));  % function to call
